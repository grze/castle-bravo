...
#
# Mathias Gug: Add timeout to getProcessOutput
#   Kills the process after some time
...
def _killProcess(instance):
    if instance.pid:
        instance.signalProcess("KILL")

def _callProtocolWithDeferred(protocol, executable, args, env, path, timeout,
                              reactor=None):
    if reactor is None:
    ...
    d = defer.Deferred()
    p = protocol(d)
    i = reactor.spawnProcess(p, executable, (executable,)+tuple(args), env, path)
    if timeout:
        reactor.callLater(timeout, _killProcess, i)
    return d
...
def getProcessOutput(executable, args=(), env={}, path=None, reactor=None,
    ...
    return _callProtocolWithDeferred(lambda d:
                                     ...
                                     executable, args, env, path, timeout,
                                     reactor)
